...
function App() {
  ...
  const toggleTodo = (id, isSubTask = false, parentId = null) => {
    setTodos((prevTodos) => {
      const newTodos = prevTodos.map((todo) => {
        if (isSubTask && todo.id === parentId) {
          // Find the subTask inside the parent task
        ...
        return todo; // Return the task unchanged if no match
      });

      return newTodos;
    });
  };
  // Reorder todos after drag
    ...
    <DndProvider backend={isMobile() ? TouchBackend : HTML5Backend}>
      <>
        <div className='z-100 conatiner'>
          ...
          <TodoLists todos={todos.slice().reverse()} toggleTodo={toggleTodo} moveTodo={moveTodo} setTodos={setTodos} />
          <button onClick={completeTodos} className="mt-4 secondary-btn">Remove all checked todo</button>
